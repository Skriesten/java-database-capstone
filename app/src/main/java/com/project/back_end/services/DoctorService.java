...
public class DoctorService {
    ...
    // #4This method fetches the available slots for a specific doctor on a given date.
    public List<Appointment> getDoctorAvailability(Long doctorId, LocalDate date) {
        ...
        doctorAppointments =  appointmentRepository.findByDoctorIdAndAppointmentTimeBetween(doctorId,startDate.atStartOfDay(), endDate.atStartOfDay());
    ...
    public int  saveDoctor(Doctor doctor) {
            ...
            if(name.equalsIgnoreCase(doctorRepository.findByNameLike(name).toString())) {
                return 0;
            }
            else {
    ...
    public ResponseEntity<Map<String, String>> validateDoctor(Login login) {
            ...
            if(doctorRepository.findbyEmail(email).getActive()) {
                token = String.valueOf(tokenService.generateToken("doctor"));
    ...
    public Map<String, Object> findDoctorByNameSpecialtyAndTime(String name, String specialty, String amOrPm) {
        ...
        map = (Map<String, Object>) doctorRepository.findByNameContainingIgnoreCaseAndSpecialtyIgnoreCase(name, specialty);
        if(map.containsValue("AM")){
            map.put("AM", amOrPm);
        }
        else {
            map.put("PM", amOrPm);
    ...
    // #12
    @Transactional
    public List<Doctor> findDoctorByTime(List<Doctor> doctors, String amOrPm) {
        //Filter doctors based on their available times, comparing the time slots with AM/PM.
        List<Doctor> doctorList = new ArrayList<>();
        Stream<Doctor> filteredDoctorStream = (Stream<Doctor>) doctorList.stream().filter(
...
//    - The `DoctorService` class depends on `DoctorRepository`, `AppointmentRepository`
//              and `TokenService`.
//    - These dependencies should be injected via the constructor for proper
//          dependency management.
//    - Instruction: Ensure constructor injection is used for injecting dependencies
//          into the service.
...
//    - Instruction: Add the `@Transactional` annotation above the methods
//              that perform database operations or queries.


// 4. **getDoctorAvailability Method**:
//    - Retrieves the available time slots for a specific doctor on a particular
//              date and filters out already booked slots.
//    - The method fetches all appointments for the doctor on the given date
//              and calculates the availability by comparing against booked slots.
...
// 5. **saveDoctor Method**:
//    - Used to save a new doctor record in the database after checking if a
//              doctor with the same email already exists.
//    - If a doctor with the same email is found, it returns `-1` to indicate conflict;
//              `1` for success, and `0` for internal errors.
//    - Instruction: Ensure that the method correctly handles conflicts and
//              exceptions when saving a doctor.


// 6. **updateDoctor Method**:
//    - Updates an existing doctor's details in the database. If the doctor
//              doesn't exist, it returns `-1`.
//    - Instruction: Make sure that the doctor exists before attempting to save
//              the updated record and handle any errors properly.
...
//              with that doctor.
//    - It first checks if the doctor exists. If not, it returns `-1`; otherwise, it
//              deletes the doctor and their appointments.
//    - Instruction: Ensure the doctor and their appointments are deleted properly,
//              with error handling for internal issues.


// 9. **validateDoctor Method**:
//    - Validates a doctor's login by checking if the email and password match
...
//    - This method is annotated with `@Transactional` to ensure that the
//              database query and data retrieval are properly managed within a transaction.
//    - Instruction: Ensure that available times are eagerly loaded for the doctors.


// 11. **filterDoctorsByNameSpecialtyAndTime Method**:
//    - Filters doctors based on their name, specialty, and availability during
//              a specific time (AM/PM).
//    - The method fetches doctors matching the name and specialty criteria,
//              then filters them based on their availability during the specified time period.
//    - Instruction: Ensure proper filtering based on both the name and specialty
//              as well as the specified time period.


// 12. **filterDoctorByTime Method**:
//    - Filters a list of doctors based on whether their available times match
//              the specified time period (AM/PM).
//    - This method processes a list of doctors and their available times to
//              return those that fit the time criteria.
//    - Instruction: Ensure that the time filtering logic correctly handles both
//              AM and PM time slots and edge cases.


// 13. **filterDoctorByNameAndTime Method**:
//    - Filters doctors based on their name and the specified time period (AM/PM).
//    - Fetches doctors based on partial name matching and filters the results
//              to include only those available during the specified time period.
//    - Instruction: Ensure that the method correctly filters doctors based on
//              the given name and time of day (AM/PM).
